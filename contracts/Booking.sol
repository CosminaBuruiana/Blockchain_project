...
contract Booking
{
    ...
    function getPricePerRoom(uint32 hotelId) public view returns(uint256) {
        return hotels[hotelId].pricePerRoom;
    }
    ...
    function getReservationStatus(uint32 reservationId) public view returns(string memory) {
        return clients[msg.sender].status[reservationId];
    }
    ...
    function makeReservation(uint32 hotelId, uint32 numberOfRooms) public onlyClient {
        ...
        require(msg.sender.balance >=
                numberOfRooms * hotels[hotelId].pricePerRoom,
                Strings.concatenateStrings("Insufficient balance: ", Strings.toString(msg.sender.balance)));
    ...
    function payReservation(uint32 reservationId) public payable onlyClient {
        require(msg.value >=
                reservations[reservationId].numberOfRooms * hotels[reservations[reservationId].hotelId].pricePerRoom,
                "Insufficient balance to pay for reservation!");
